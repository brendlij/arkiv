...
Assigned folders are read-only views of manually managed source directories. Arkiv writes only to its configured managed-original directory and does not modify manually copied originals. Overlapping grants intentionally allow multiple users to see the same originals; each user's favorites remain private.

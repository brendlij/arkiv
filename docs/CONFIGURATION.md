...
| Variable                   | Default                     | Purpose                                                                                          |
| -------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------ |
| `ARKIV_DATABASE`           | `/data/gallery.db`          | Absolute SQLite path; uploaded originals live in the adjacent `uploads/` directory               |
...
| `ARKIV_UPLOAD_QUOTA_BYTES` | `5368709120`                | Default per-user upload quota (5 GiB); per-user overrides in People & access                     |
...
The default Compose file exposes the ordinary user settings and fixes server paths and port to match its mounts. Advanced settings must be added to the service's `environment` block; putting an arbitrary variable into `.env` alone does not pass it to the container. `VIPS_CONCURRENCY=1` is a native libvips setting supplied by the image, not an arkiv variable.
...
Merge this example into the existing service; retain its data, cache and photos mounts. IDs must be stable and unique, and roots must not overlap. The managed upload library is added automatically. Changing a root under an existing ID is rejected to protect associations. Disabling a library hides it without discarding its metadata.
